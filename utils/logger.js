...
const ACTIVE_LOG_PATH = path.join(LOG_DIR, "webhook.log");
...
function getRotatedLogPath(dateKey) {
  const basePath = path.join(LOG_DIR, `webhook-${dateKey}.log`);
  ...
  let counter = 1;
  while (true) {
    const candidate = path.join(LOG_DIR, `webhook-${dateKey}-${counter}.log`);
